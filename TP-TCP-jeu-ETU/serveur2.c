...
int main(int argc, char **argv) {
    ...
    while (1){
        ...
        /* création d'un processus fils pour pouvoir gérer les clients */

        if (client_sock>0 && fork()==0){

            printf("[%d]: connection acceptée\n", (int)getpid());
            ...
            if (argc == 2 && strcmp(argv[1],"rand")==0) {
                ...
                x_tresor = 1 + rand()%N;
                y_tresor = 1 + rand()%N;
                printf("[%d]: position trésor : %d,%d\n", (int)getpid(), x_tresor,y_tresor);
            }

            /* création d'une boucle dans le processus fils */
            ...
            while (res!=0)
            {
                ...
                printf("[%d]: message envoyé : %s\n",(int)getpid(), serialisation);
